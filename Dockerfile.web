...
MAINTAINER "Anton Matrenin" <matrenin@ukr.net>

LABEL Description="This image is based on Solita's Centos7 w/ systemd image, and runs R Studio Server Pro 1.1.383 over port 8787. To run the container, use the run command: docker run --name rstudio-server -t -d --security-opt seccomp=unconfined --stop-signal=SIGRTMIN+3 --tmpfs /run --tmpfs /run/lock -v /sys/fs/cgroup:/sys/fs/cgroup:ro -p 8787:8787 sqldiablo/rstudio-server:1.1.383 To get to a shell prompt once the container is running, use the following command: docker exec -u 0 -it rstudio-server bash "
LABEL Product="R Studio Server Pro" Version="1.1.383"
...
RUN yum install --nogpgcheck -y https://download2.rstudio.org/rstudio-server-rhel-pro-1.1.383-x86_64.rpm
